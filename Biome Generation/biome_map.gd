...
func _update_maps():
	...
	var simplex_elevation = Helpers._noise_to_texture(size, SimplexTexture.new(), Elevation.base_simplex)
	var simplex_temperature = Helpers._noise_to_texture(size, SimplexTexture.new(), Climate.temperature_simplex)
	var simplex_precipitation = Helpers._noise_to_texture(size, SimplexTexture.new(), Climate.precipitation_simplex)
	var simplex_humidity = Helpers._noise_to_texture(size, SimplexTexture.new(), Climate.humidity_simplex)
	
	var perlin_elevation = Helpers._noise_to_texture(size, NoiseTexture2D.new(), Elevation.base_perlin)
	var perlin_temperature = Helpers._noise_to_texture(size, NoiseTexture2D.new(), Climate.temperature_perlin)
	var perlin_precipitation = Helpers._noise_to_texture(size, NoiseTexture2D.new(), Climate.precipitation_perlin)
	var perlin_humidity = Helpers._noise_to_texture(size, NoiseTexture2D.new(), Climate.humidity_perlin)
	...
	Helpers._set_biome_thresholds(simplex_shader, Config)
	...
	Helpers._set_biome_thresholds(perlin_shader, Config)
